...
- Repository: canonical/opensearch-operator (named in the request).
- Product: Charmed OpenSearch; track 2 (docs default version; both repos default branch 2/edge).
- from-ref: rev315 (opensearch-operator) and rev60 (opensearch-dashboards-operator),
  taken from the newest release-notes file in the repo's own docs tree:
  docs/reference/release-notes/revision-315.md (December 16, 2025). Both confirmed
  ancestors of 2/edge. Ranges: rev315...2/edge (49 commits), rev60...2/edge (18 commits).
- to-ref: 2/edge HEAD. opensearch/rev366's peeled commit (73177e12) IS the 2/edge
  HEAD, so this release is already tagged → title "Revision 366" (confirmed, not proposed).
  Release date September 14, 2026 from the annotated tag opensearch/rev366 (tagger date).
- Charmed OpenSearch Dashboards included as a component: detected in revision-315's
  notes, confirmed by the user in this run. Its latest tag opensearch-dashboards/rev78
  is 2 commits behind 2/edge HEAD → this release ships a new, not-yet-tagged dashboards
  revision (expected rev79, per the per-architecture pair convention 79+80).
- K8s charm rows in the Compatibility table: the K8s charms were merged into these
  repos within this range (PRs #830 and #275); opensearch-k8s/rev16 shares commit
  73177e12 with opensearch/rev366 (both at 2/edge HEAD), opensearch-dashboards-k8s/rev5
  shares 1f5db4fe with opensearch-dashboards/rev78.
- Workload version 2.19.6 from the 2/edge channel of the opensearch and
  opensearch-dashboards snaps (Snap Store API, 2026-09-16).

Auto-sort: RAN (user-enabled). Moves (Charmed OpenSearch component):
...
Known auto-sort misses (no conventional-commit prefix, left in Other improvements):
- PR #789 "add smtp support" and PR #786 "add rollback compatibility" read as features
  but carry no feat: prefix — review manually whether to move them to Features.

Edits made to entry text (spelling/code-token fixes only, meaning untouched):
- PR #799: "acomodating" → "accommodating", "exclusivness" → "exclusiveness" (PR-title typos).
- Backticked code-like tokens so MyST linkify cannot turn them into links:
  `charm.py`, `test_charm.py`, `metadata.yaml` (opensearch-operator);
  `README.md`, `charmcraft.yaml` (dashboards).
- Dashboards entries #266, #268, #273, #275, #276, #278, #281, #282, #283 had no PR
  link in the builder output (commit-only); PR numbers verified via the GitHub API
  and links added.

TODOs for the release owner:
- Confirm the final published dashboards charm revision before publishing: the latest
  tag is opensearch-dashboards/rev78, 2 commits behind 2/edge HEAD, so this release
  ships a new, not-yet-tagged revision (expected rev79; revisions allocate in
  per-architecture pairs, so verify the pair 79+80 and which member publishes).
- Verify the opensearch/rev365 + rev366 and opensearch-dashboards/rev77 + rev78 tag
  pairings (assumed per-architecture pairs from the same release run) and which
  revision ships on which architecture.
- Fill in the snap revisions in the Compatibility table once the 2.19.6 snaps are
  published to the 2/stable channel (as of writing, 2/stable still serves 2.19.4:
  opensearch rev 98, dashboards rev 54; 2/edge serves 2.19.6: opensearch rev 261
  amd64 / 260 arm64, dashboards rev 155 amd64 / 156 arm64).
- Fill in the Artifacts cells for the K8s charms (rock/image references) or drop
  those rows if the K8s charms are documented separately.
...
Note on tag namespaces: since the single-kernel restructuring (PR #830), tags are
namespaced (opensearch/revNNN, opensearch-k8s/revN, opensearch-dashboards/revNN,
...
Note for the docs checks (when this file is copied into the opensearch-operator repo):
the fork's docs/.custom_wordlist.txt already contains the words this document needs
(GCS, toolchain, rediraffe, autogenerated, repo, READMEs), added during the previous
run of this same range.
-->
