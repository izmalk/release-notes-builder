...
8. Docs checks (`make spelling`, `make linkcheck`) were run against this page
   and both are clean. Fixes applied:
   - PR-title typos corrected: "acomodating" → "accommodating", "exclusivness"
     → "exclusiveness", "gcs" → "GCS".
   - Bare filenames wrapped in backticks (`charm.py`, `test_charm.py`,
     `metadata.yaml`, `charmcraft.yaml`, `README.md`) so MyST's linkify stops
     turning them into links. Two real consequences of not doing this:
     `charm.py` became a broken `http://charm.py`, and `README.md` became
     `http://README.md`, which *resolved* — to a domain squatter — and so
     passed linkcheck silently.
   - Added to `docs/.custom_wordlist.txt`: GCS, toolchain, rediraffe,
     autogenerated, repo, READMEs.
   No `linkcheck_ignore` entry was needed. Re-run both checks after any further
   edit to this page, and check the `[redirected ...]` lines in
